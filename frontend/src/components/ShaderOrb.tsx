...
// Sphere displaced by noise. The displacement is sampled in OBJECT space so the
// detail turns with the body instead of swimming across a fixed surface.
float map(vec3 p, mat3 rot, float t) {
  vec3 q = rot * p;
  float d = length(p) - 1.0;
  float n = fbm(q * 2.6 + vec3(0.0, 0.0, t * 0.25));
  return d - n * 0.16;
}
...
void main() {
  ...
  if (hitDist > 0.0) {
    ...
    float veins = fbm(rot * p * 5.0 + vec3(0.0, 0.0, t * 0.4));

    col  = u_tint * (0.18 + dif * 0.85);
    col += u_tint * veins * 0.55;
    col += u_tint * fres * 1.5;
    col += vec3(1.0) * spe * 0.5;
    alpha = 0.92;
  }

  // Outer glow, so the body sits in light instead of on a hard edge.
  float halo = exp(-3.1 * max(length(uv) - 0.72, 0.0));
  col   += u_tint * halo * 0.30;
